...
async function validatePaginatedTable(expect, page, grid, expectedPages, options) {
...
export function runAgGridDataSuite({ getBaseUrl, getBrowser, pagePath, versionLabel }) {
  const { expect, test } = createTestApi({ getBrowser, getBaseUrl });

  test.describe(`wdio ag-grid data scenarios (${versionLabel})`, () => {
    ...
    test("verify paginated table data - any order - include all columns", async ({ page }) => {
      const grid = createAgGrid(await getBrowser().$(agGridSelector));
      await validatePaginatedTable(expect, page, grid, expectedPaginatedTableData);
    });
    ...
    test("verify paginated table data - excluding columns", async ({ page }) => {
      ...
      await validatePaginatedTable(expect, page, grid, expectedSubset, {
